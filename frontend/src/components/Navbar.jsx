...
const Navbar = () => {
    ...
    // BOUNCE THE CART ITEM WHEN ADD TP CART AN ITEM
    useEffect(() => {
    ...
    // [cartCount] is the dependency array.It tells React: "Run this effect whenever cartCount changes"
    ...
    return (
        ...
        >
                        ...
                        {navItems.map((item) => (
                            ...
                            >
                                <div className="flex items-center">
                                    <span
                                        className={`${navbarStyles.navIcon} ${activeTab === item.path ? navbarStyles.activeNavIcon : navbarStyles.inactiveNavIcon}`}
                                    >
                                        {item.icon}
                                    </span>
                                    <span>{item.name}</span>
                    ...
                    <div className={navbarStyles.iconsContainer}>
                        {isLoggedIn ? (
                            ...
                            <Link to='/login' className={navbarStyles.loginLink}>
                                <FiUser className={navbarStyles.loginLink} />
            ...
            ${isOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'} fixed inset-0 bg-black bg-opacity-50 transition-opacity duration-300`}
                onClick={() => setIsOpen(false)}>
                <div className={`${navbarStyles.mobilePanel} ${isOpen ? 'translate-x-0' : 'translate-x-full'} fixed right-0 top-0 bottom-0 z-50 w-4/5 max-w-sm`}
                        ...
                        {navItems.map((item, idx) => (
                            <Link key={item.name} to={item.path}
                                ...
                                style={{
                                    ...
                                    opacity: isOpen ? 1 : 0,
                                    // When menu opens -> opacity = 1 (fully visible)
                                    // When menu closes -> opacity = 0 (invisible)
