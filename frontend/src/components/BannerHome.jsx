...
const BannerHome = ({ onSearch }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (trimmedTerm) {
            if (onSearch) {
                const searchWords = trimmedTerm.toLowerCase().split(/\s+/) // splits by whitespace and formed into an array
                ...
                // example:URL becomes /items?search=fresh%20groceries
                // setSearchTerm('') Clears the search input after submission
                    ...
                    <div className='text-center md:text-left'>
                        <div className={bannerStyles.tag}>
                            <span className='flex items-center text-sm sm:text-base'>
                                {/* text=-base = sets font sizze to 16px (base/default size) */}
                        ...
                        <form onSubmit={handleSubmit} className={bannerStyles.form}>
                            <input type="text" value={searchTerm} onChange={handleSearch} placeholder='Search for fruits, vegetables, meats, dairy...' className={bannerStyles.input} />
